...
public class kblh {
	...
	public static class Gematria {
        ...
        public int getGematria(Mispar mispar){
            return GetGematria(mispar.getArray());

        }
        private int GetGematria(int[] table) {
            int g = 0;
            for (char c : Text.toCharArray()) {
                int i = c - 0x05d0;
        ...
        public String ChilufiOtiot(Chilufi ChilufiMethod) {
            int[] tbl = ChilufiMethod.getArray();
            return GetChilufi(tbl);
        }

        public String ChilufiOtiot(AlefBet table) {
            int[] tbl = table.ToArray();
            return GetChilufi(tbl);
        }

        public String ChilufitOtiot(AlefBet table) {
            int[] tbl = table.ToArray();
            return GetChilufi(tbl);
        }

        private String GetChilufi(int[] tbl) {
            String res = "";
            for(char c : Text.toCharArray())
            {
                int i = c - 0x05d0;
